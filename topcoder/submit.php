				...
				if(!$msg){
					if($value == 15){
						...
						if($count==0){
							...
							$score = $score + $points;
							mysql_query("UPDATE  topcoders SET score=$score WHERE topcoder_id=$id") or die (mysql_error());
						}
						else
						{
...
<?php
	include "header.php";
?>
<font size="3"><strong>Contest Submissions</strong></font><br><br>
<?php
	if($msg)
	echo "<strong>$msg</strong><br><br><br><a href='submit.php'>Go to Submission form -></a>";
	else{
?>
	...
	<?php
		...
		if($count==0)
			...
			for($i = 0; $i < $count; $i++){
				...
				if($tempID == 1)
					echo "<option value='$actualID' selected='selected'>Problem ID $tempID</option>";
				else
					echo "<option value='$actualID'>Problem ID $tempID</option>";
			}
			echo "</select>";
		}
	?>
